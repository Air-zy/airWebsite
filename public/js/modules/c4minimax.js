...
async function minmax(distance, depth, isMaxer, alpha, beta) {
  ...
  if (isMaxer) {
    ...
    for (let i = 0; i < moves.length; ++i) {
      rawDrop(moves[i]);
      let [score, bmove] = await minmax(distance+1, depth - 1, false, alpha, beta);
      rawUndrop();
      if (score > max) {
        bestMove = moves[i];
        max = score;
    ...
    transTable.set(zobKey, max, bestMove, depth);
    return [max, bestMove];
  } else {
    let min = myHuge;
    for (let i = 0; i < moves.length; ++i) {
      rawDrop(moves[i]);
      let [score, bmove] = await minmax(distance+1, depth - 1, true, alpha, beta);
      rawUndrop();
      if (score < min) {
        bestMove = moves[i];
        min = score;
    ...
    transTable.set(zobKey, min, bestMove, depth);
    return [min, bestMove];
  }
}
